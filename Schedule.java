...
public class Schedule implements ScheduleInterface {
    ...
    public boolean addTask(String name, String type, double startTime, double duration, int startDate) {
        if (type.equalsIgnoreCase("cancellation")) {
            // Before adding an anti-task, ensure there is a matching recurring occurrence
            if (!canAntiTaskApply(name, startDate, startTime, duration)) {
                System.out.println("Error: No matching recurring task occurrence found for this cancellation.");
                return false;
            }

            // Anti-task
            TaskInterface newTask = new Anti(name, type, startTime, duration, startDate, null);
            // No overlap check is needed because an anti-task cancels an occurrence, it doesn't cause overlapping issues
            tasks.add(newTask);
            return true;
        } else {
            // Transient task
            TaskInterface newTask = new Transient(name, type, startTime, duration, startDate);
    ...
    public boolean addTask(String name, String type, double startTime, double duration, int startDate, int endDate, int frequency) {
        ...
        // Generate occurrences temporarily for overlap checking
        ...
        // If no overlaps, add only the Recurring object itself
        tasks.add(newTask);
        return true;
    }
    ...
    public boolean checkOverlap(TaskInterface newTask) {
        for (TaskInterface task : tasks) {
            // Skip Anti-tasks during overlap checks, as they don't represent actual scheduled work
        ...
        for (TaskInterface t : displayList) {
            if (t instanceof Anti) {
                ...
                if (blockedOccurrence != null) {
                    ...
                    // If no matching occurrence found, you may choose to remove anti-tasks or not.
                    // Currently, if no matching occurrence is found, the anti-task does nothing.
                }
            }
        }
        ...
        for (TaskInterface occ : list) {
            if (!(occ instanceof Recurring) && !(occ instanceof Anti)) {
                if (occ.getName().equalsIgnoreCase(anti.getName()) &&
                    ...
                    Math.abs(occ.getDuration() - anti.getDuration()) < 0.0001) {
                    return occ;
                }
            }
        }
        ...
        try {
            ...
            for (String obj : objects) {
                ...
                if (type.equalsIgnoreCase("Cancellation")) {
                    ...
                    if (!canAntiTaskApply(name, date, st, du)) {
                        System.out.println("Error: No matching recurring task occurrence for this anti-task from file.");
                        // Do not add
                    } else {
                    ...
                    // Check overlap if you want (not mandatory), or just add
                    if (checkOverlap(new Transient(name, type, st, du, d))) {
                        System.out.println("Error: Overlapping transient task from file. Not added.");
                    } else {
    ...
    /**
     * Checks if an anti-task can apply. That is, if there is a recurring task that has an occurrence
     * with the given date, start time, and duration.
     */
    private boolean canAntiTaskApply(String antiName, int date, double startTime, double duration) {
        for (TaskInterface t : tasks) {
            if (t instanceof Recurring) {
                Recurring r = (Recurring) t;
                for (TaskInterface occ : r.getOccurrances()) {
                    // Match if same date, start time, duration, and name
                    // The instructions say the anti-task must cancel one repetition of a recurring task 
                    // that presumably matches name, date, startTime, and duration. 
                    if (occ.getName().equalsIgnoreCase(antiName) &&
                        occ.getStartDate() == date &&
                        Math.abs(occ.getStartTime() - startTime) < 0.0001 &&
                        Math.abs(occ.getDuration() - duration) < 0.0001) {
                        return true;
                    }
                }
            }
